...
Future<List<GaragesRecord>> availableGarages(
  ...
  var garages = await queryGaragesRecord(
      queryBuilder: (garagesRecord) => garagesRecord
          ...
          .where('isCreationFinished', isEqualTo: true)).first;
  ...
  for (GaragesRecord garagesRecord in availableGaragesConsideringWorkingHours) {
    var garageReservations = await queryReservationRecord(
      queryBuilder: (reservationRecord) => reservationRecord
          .where('garage_reference', isEqualTo: garagesRecord.reference)
          .where('isCreationFinished', isEqualTo: true),
    ).first;

    bool isTaken = false;
    for (ReservationRecord reservation in garageReservations)
      if (compareDatesOnly(reservation.dateReservation!,
              reservationRecord.dateReservation!) &&
          isTimeInRange(
...
bool compareDatesOnly(DateTime firstDate, DateTime secondDate) {
  DateTime firstDateOnly =
      DateTime(firstDate.year, firstDate.month, firstDate.day);
  DateTime secondDateOnly =
